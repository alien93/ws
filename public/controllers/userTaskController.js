angular.module('tsApp')
        .controller('userTaskController', function($scope, $routeParams, $uibModal){
            ...
            var task = {"code":"code", "project":project, "taskVersions": []};
           
            //-------------------\test data------------------
           
            $scope.projects = [project];
            $scope.task = task;
            $scope.task.taskVersions.push($scope.projects[idProj].tasks[idTask]);
            $scope.task.taskVersions.push($scope.projects[idProj].tasks[1]);
